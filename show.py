...
def _plot_real_and_fake_images(current_epoch, real_batch, device, img_list, save_path):
    ...
    name = "1real_and_fake_%d.jpg" % (current_epoch+1)
    full_path_name = "%s/%s" % (save_path, name)
    plt.savefig(full_path_name)
    # plt.show()


def _show_generator_images(current_epoch, G_losses, D_losses, save_path):
    ...
    name = "1G_D_losses_%d.jpg" % (current_epoch+1)
    full_path_name = "%s/%s" % (save_path, name)
    plt.savefig(full_path_name)
    # plt.show()
...
def show_images(train_model, config, dataloader):
    ...
    current_epoch = train_model["current_epoch"]
    save_path = record.get_check_point_path(config)
    _show_generator_images(current_epoch, G_losses, D_losses, save_path)
    # _save_img_list(img_list, save_path, config)
    real_batch = next(iter(dataloader))
    _plot_real_and_fake_images(current_epoch, real_batch, config["device"], img_list, save_path)
